...
library(lmtest)
library(plm)
my_data <- read_excel("C:/Users/Ketki/Downloads/Qdapp dataset (1).xlsx")  #Upload the data
#filtered_data
...
pool<-plm(Life_expectancy ~  percentage_expenditure+Adult_Mortality+Alcohol+Hepatitis_B+Polio+Measles+Diphtheria+BMI+Total_expenditure+HIVAIDS+Schooling,data=my_data,model="pooling")
summary(pool)

pool<-plm(Life_expectancy ~  percentage_expenditure+Adult_Mortality+Polio+Diphtheria+BMI+HIVAIDS+Schooling,data=my_data,model="pooling")
summary(pool)

pool<-plm(Life_expectancy ~  percentage_expenditure+Adult_Mortality+Diphtheria+BMI+HIVAIDS+Schooling,data=my_data,model="pooling")
summary(pool)
